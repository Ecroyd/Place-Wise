...
const demoPrompt = "I work in Middleton five days a week. My partner works from home. We have a £1 million budget and want somewhere fairly rural with good schools, green space and no more than about 35 minutes to work.";
...
const money = (value:number) => new Intl.NumberFormat("en-GB",{style:"currency",currency:"GBP",maximumFractionDigits:0,notation:value >= 1_000_000 ? "compact" : "standard"}).format(value);
...
export function HeatMapApp() {
  ...
  async function run() {
    setLoading(true);
    try {
      const criteria = await new DeterministicCriteriaParser().parse(inputMode === "ai" ? text : `Middleton five days, 35 minutes, £${budget} budget, schools and green space`);
      criteria.mode = mode;
      if (inputMode === "manual") criteria.property = {maximumBudget:budget,hardBudget:false};
  ...
  return <main className="shell"><Nav/><section className="hero"><span className="eyebrow">Area recommendations, made personal</span><h1 className="serif">Where does your life<br/><em>fit best?</em></h1><p className="sub">Tell us what matters — commutes, budget, schools and green space. We’ll turn your priorities into a heat map of the best areas.</p><div className="mode-switch"><button className={mode === "live" ? "active" : ""} onClick={()=>setMode("live")}>Where should I live?</button><button className={mode === "work" ? "active" : ""} onClick={()=>setMode("work")}>Where should I work?</button></div><div className="search-card"><div className="input-tabs"><button className={inputMode === "ai" ? "active" : ""} onClick={()=>setInputMode("ai")}>✦ Describe your ideal area</button><button className={inputMode === "manual" ? "active" : ""} onClick={()=>setInputMode("manual")}>Adjust priorities</button></div>{inputMode === "ai" ? <textarea className="prompt" value={text} onChange={e=>setText(e.target.value)} aria-label="Describe your ideal area"/> : <div className="manual"><div className="field"><label htmlFor="destination">Main destination</label><input id="destination" defaultValue="Middleton"/></div><div className="field"><label htmlFor="commute">Preferred commute</label><select id="commute" defaultValue="35"><option value="35">35 minutes</option><option value="45">45 minutes</option></select></div><div className="field"><label htmlFor="budget">Maximum budget</label><input id="budget" type="number" value={budget} onChange={e=>setBudget(Number(e.target.value))}/></div><Weight label="Schools" value={schools} setValue={setSchools}/><Weight label="Green space" value={greenSpace} setValue={setGreenSpace}/></div>}<div className="prompt-foot"><small>Affordability uses historical area estimates — not live listings</small><button className="go" onClick={run} disabled={loading}>{loading ? "Building your heat map…" : "Show my heat map"}<span>→</span></button></div></div><p className="demo-note">Demo area, travel-time and historical-price estimates are mock data — not property advice.</p></section><section className="proof"><div><b>Area-level affordability</b>Compare your budget with historical sold-price estimates.</div><div><b>A real heat map</b>Every cell represents the combined strength of your priorities.</div><div><b>No property listings</b>Listings can be added later without changing area optimisation.</div></section></main>;
}
