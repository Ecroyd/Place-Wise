...
export function GoogleAreaMap({ destinations, selectedArea, onPointSelected, budget=500000, minimumBudget=0, onModeChange, combinedMatches=[] }: { destinations: DestinationConstraint[]; budget?:number; minimumBudget?:number; onModeChange?:(mode:TransportMode,index:number)=>void; combinedMatches?:CombinedMatch[]; selectedArea?: Coordinates & {minutes:number|null;selectedMode?:TransportMode}; onPointSelected?: (point:Coordinates)=>void }) {
      ...
      try {
        ...
        map.on("load", () => {
          ...
          const fitArea = (duration = 600) => {
            ...
            map.stop();
            map.fitBounds(bounds, {padding:{top:Math.min(265,height*.27),bottom:Math.min(275,height*.27),left:Math.min(90,width*.14),right:Math.min(90,width*.14)},maxZoom:12,duration}, {commuteAutoFit:true});
          };
  ...
  return <>
    ...
    <MapLayers map={mapInstance} budget={budget} minimumBudget={minimumBudget}/>
    <div className="heat-callout"><span className="eyebrow">Your travel-time area</span><h2>{selected == null ? `${allDestinations?"Shared commute area":limits.minimum+"–"+limits.maximum+" minutes"}` : `${selected}${allDestinations?"% of time limit":" minutes"}`}</h2><p>{selected === null ? allDestinations?"No shared commute match at this sample point.":"Journey time unavailable at this point." : `One-way journeys to ${destination?.label ?? "your destination"}.`}</p>{selectedMode&&<small><b>{destination?.transportMode==="any"?"Fastest available: ":"Travel mode: "}{modeLabel(selectedMode)}</b></small>}{journey.length>0&&<ol className="journey-steps" aria-label="Journey steps">{journey.map((step,index)=><li key={index}><b>{step.mode}{step.line?` ${step.line}`:""} · {step.minutes} min</b>{(step.from||step.to)&&<span>{step.from}{step.to?` → ${step.to}`:""}</span>}</li>)}</ol>}<small>Shading follows routed journey times across council boundaries. Click the map to inspect a sample. Times are sampled estimates, not exact address-level contours.</small><small>Travel time only; property and distance preferences apply to the comparisons alongside.</small></div>
