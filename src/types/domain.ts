...
export interface LocationCell extends Coordinates { h3Index:string; town:string; region:string; typicalHousePrice:number; scores:Partial<Record<PreferenceKey,number>>; employmentDensity:number; dataQuality:number }
export interface DestinationResult { destinationId:string; minutes:number; distanceKm?:number; weeklyMinutes:number; preferredDelta:number }
